...
class NetFense:
    ...
    def gradient_wrt_x(self, label):#np.array [N, D] matrix containing the gradients.
        return self.adj_preprocessed.dot(self.adj_preprocessed)[self.u].T.dot(self.W[:, label].T)
    def gradient_wrt_xp(self, label):#np.array [N, D] matrix containing the gradients.
        return self.adj_preprocessed.dot(self.adj_preprocessed)[self.u].T.dot(self.Wp[:, label].T)
    ...
    def compute_logitsp(self):#np.array, [N, K]            The log probabilities for each node.
        return self.adj_preprocessed.dot(self.adj_preprocessed).dot(self.X_obs.dot(self.Wp))[self.u].toarray()[0]
        ...
        for _ in range(n_perturbations):
            ...
            if perturb_structure:
                ...
                if filtered_edges_final.shape[0]:
                  print('->filtered_edges_final ', filtered_edges_final.shape[0])
            ...
            if change:
              if perturb_structure and perturb_features:
                  ...
                  if 1:#best_edge_score < best_feature_score:
                      if self.verbose:
                          print("Edge perturbation: {}".format(best_edge))
                          print('best_edge_score: ', np.round(best_edge_score, 2))
